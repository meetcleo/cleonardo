...
Blocked on COREEXP-325 landing: the publish step needs `packages: write` actually granted — the one ask that's unaffected by repo visibility. `cleonardo` is now public (further than the private → internal flip COREEXP-325 originally asked for), which already settles Dependabot's ability to read this repo's release notes and clone the gem's git source — see `docs/publishing-access.md` for the full breakdown of which asks that leaves open.
