...
> **Known mismatch:** as committed, the COREEXP-264 reader PRs (#29, #30) key their `walk` off a plain `value` field, not `$value` — since `walk` skips `$`-prefixed keys, they'll build an empty lookup against the shape this package emits. Tracked to fix on those PRs, not here.
